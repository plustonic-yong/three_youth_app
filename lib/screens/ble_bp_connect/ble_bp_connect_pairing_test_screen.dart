...
class BleBpConnectPairingTestScreen extends StatefulWidget {
  const BleBpConnectPairingTestScreen({Key? key}) : super(key: key);

  @override
  State<BleBpConnectPairingTestScreen> createState() =>
      _BleBpConnectPairingTestScreenState();
}

class _BleBpConnectPairingTestScreenState
    extends State<BleBpConnectPairingTestScreen> {
  bool isLoading = true;
  // ignore: unused_field
...
// These are the UUIDs of your device
  ...
  void initState() {
    Future.delayed(Duration.zero, () async {
      ...
      //   context.read<BleBpConnectProvider>().startPairing();
      // });
      // startPairing();
      ...
      setState(() {});
      await context.read<BleBpConnectProvider>().loadCounter();
    });
  ...
  Widget getAppBar() {
    return AppBar(
      leading: GestureDetector(
        onTap: () async {
          await context.read<BleBpConnectProvider>().disConnectPairing();
  ...
  Widget build(BuildContext context) {
    _isPairing = context.read<BleBpConnectProvider>().isPairing;
    _isPaired = context.watch<BleBpConnectProvider>().isPaired;
    _isScanning = context.read<BleBpConnectProvider>().isScanning;
    _dtStart = context.read<BleBpConnectProvider>().dtStart;
    _foundDeviceWaitingToConnect =
        context.read<BleBpConnectProvider>().foundDeviceWaitingToConnect;
    _connected = context.read<BleBpConnectProvider>().connected;
    _dtStart = context.read<BleBpConnectProvider>().dtStart;
    _scanStream = context.read<BleBpConnectProvider>().scanStream;
    _dtStart = context.read<BleBpConnectProvider>().dtStart;
    _dtStop = context.read<BleBpConnectProvider>().dtStop;
    _testID = context.read<BleBpConnectProvider>().testID;
    _timer = context.read<BleBpConnectProvider>().timer;
    _dtStop = context.read<BleBpConnectProvider>().dtStop;
    _iNeedDisconnect = context.read<BleBpConnectProvider>().iNeedDisconnect;
    _iFindCnt = context.read<BleBpConnectProvider>().iFintCnt;
    _ubiqueDevice = context.read<BleBpConnectProvider>().ubiqueDevice;
    _rxCharacteristic = context.read<BleBpConnectProvider>().rxCharacteristic;
    _txCharacteristic = context.read<BleBpConnectProvider>().txCharacteristic;
    _ssRx = context.read<BleBpConnectProvider>().ssRx;
    _isTestSave = context.read<BleBpConnectProvider>().isTestSave;
    _isResult = context.read<BleBpConnectProvider>().isResult;
    _dataCnt = context.read<BleBpConnectProvider>().dataCnt;
    _dataIsOK = context.read<BleBpConnectProvider>().dataIsOK;
    _dataState = context.read<BleBpConnectProvider>().dataState;
    _isUpdated = context.read<BleBpConnectProvider>().isUpdated;
    _lDataSYS = context.read<BleBpConnectProvider>().lDataSYS;
    _lDataDIA = context.read<BleBpConnectProvider>().lDataDIA;
    _lDataPUL = context.read<BleBpConnectProvider>().lDataPUL;
    return Scaffold(
  ...
  //         });
  //         debugPrint(
  //             '##################### $_isPaired $_isPairing $_foundDeviceWaitingToConnect $_connected');
  //         if ((_isPaired == false && _isPairing == true) || _isPaired) {
  ...
  //   sql += " '$testID', '${ff.format(_dtStart)}', '${ff.format(_dtStop)}'";
  ...
  //       String fileSave = dfSave.format(_dtStart) + '.txt';
  ...
  //   sql += " where eq = '$testID' ";
  //   sql += " and start = '${ff.format(_dtStart)}' ";
  //   sql += " and stop = '${ff.format(_dtStop)}'";
