...
import 'package:three_youth_app/history_screen/history_appscreen.dart';
...
class BleBPConnectScreen extends StatefulWidget {
  ...
  static BleBPConnectScreenState? of(BuildContext context) =>
      context.findAncestorStateOfType<BleBPConnectScreenState>();
  @override
  BleBPConnectScreenState createState() => BleBPConnectScreenState();
}

class BleBPConnectScreenState extends State<BleBPConnectScreen> {
  bool isLoading = true;
  // ignore: unused_field
...
// These are the UUIDs of your device
  ...
  _loadCounter() async {
    ...
    print('-------------------tiemr ON tttttttttttttttttttttttttttttttttttttt');
    _timer = Timer.periodic(const Duration(milliseconds: 1000), (timer) async {
      if (iNeedDisconnect > 0) {
        iNeedDisconnect--;
        if (iNeedDisconnect == 0) {
          print("################ FORCE CLOSE BLE ###########");
          setState(() {
            _isScanning = false;
            if (_scanStream != null) {
              try {
  ...
  showAlertDialog(BuildContext context) {
    ...
    // set up the AlertDialog
    AlertDialog alert = AlertDialog(
      title: Text("이미 기기가 페어링 되어 있습니다."),
      content: Text("페어링 해지를 원하시면 연결해지를 클릭하세요."),
      actions: [
        continueButton,
        cancelButton,
      ],
  ...
  showPairFinishDialog(BuildContext context) {
    ...
    AlertDialog alert = AlertDialog(
      title: Text("페어링 완료"),
      content: Text("페어링이 완료 되었습니다. 페어링 이후에는 측정 데이터는 자동 인식 됩니다."),
      actions: [
        continueButton,
      ],
    );
  ...
  showPairAlertDialog(BuildContext context) {
    ...
    AlertDialog alert = AlertDialog(
      title: Text("페어링 시작하기"),
      content: Text("방법 : 기기의 START 버튼을 3초이상 누르면 Pr이 표시됩니다."),
      actions: [
        continueButton,
        cancelButton,
      ],
  ...
  Future<void> doTestSave() async {
    ...
    if (sr != 'TRUE' || isDemoSaveLocal == 1) {
      ...
      try {
        ...
        CubeStorage cs = new CubeStorage(fileSave);
